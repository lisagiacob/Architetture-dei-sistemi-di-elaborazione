...
uint32_t init_timer ( uint8_t timer_num, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
	...
	LPC_TIM0->MCR = 3;		//stop, Reset, Interrupt <- 011 -> don't stop, go ciclic; Reset; Interrupt, do the code in IRQ
...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
		LPC_TIM0->PR = Prescaler;	//dilata la frequenza di clock
		
		if(MatchReg == 0)		//esistono 4 match register da 32 bit ciascuno
		{
			LPC_TIM0->MR0 = TimerInterval;
	  ...
	  else if ( timer_num == 3 )
  {
	...
	//NVIC_SetPriority(TIMER3_IRQn, 4);		// less priority than buttons 
		NVIC_SetPriority(TIMER3_IRQn, 0);		/7 more priority than buttons 
		return (0);
	}
  return (1);
}*/
